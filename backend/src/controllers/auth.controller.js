...
const forgotPassword = catchAsync(async (req, res, next) => {
  ...
  if (!findUser)
    return next(new AppError(`User not found for email: ${email}`, 404));
  ...
  try {
    ...
    return res.status(200).json({
      status: 'success',
      message: `url to reset token sent to your email ${findUser.email}`
    });
  } catch (error) {
...
const resetPassword = catchAsync(async (req, res, next) => {
  ...
  if (!password || !passwordConfirm)
    return next(
      new AppError('password and passwordConfirm both are required', 400)
    );
...
const updatePassword = catchAsync(async (req, res, next) => {
  ...
  if (
    !(await findUser.comparePassword(
